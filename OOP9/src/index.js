...
import { studentService, groupService, programService } from './services';
...
class StudentEdit extends Component {
  ...
  render() {
    if (!this.student) return <div>loading student edit...</div>;
      ...
      <div>
        <Card title="Edit student">
          ...
          <Form.Select
            ...
            onChange={(event) => (this.student.group_id = event.currentTarget.value)}
          >
...
class StudentNew extends Component {
  student = { name: '', email: '', program: '', code: '', group_id: 1 };
  groups = [];
...
class GroupDetails extends Component {
  ...
  render() {
    if (!this.group) return <div>Loading groups...</div>;
...
class GroupEdit extends Component {
  ...
  render() {
    if (!this.group) return 'Loading groups...';
      ...
      <div>
        <Card title="Edit group">
          ...
          <Form.Input
            ...
            src={this.group.img}
            onChange={(event) => (this.group.img = event.currentTarget.value)}
          />
  ...
  mounted() {
    groupService.getGroup(this.props.match.params.group_id, (group) => (this.group = group));
    groupService.getStudents(
      this.props.match.params.group_id,
      (students) => (this.students = students)
    );
...
class GroupNew extends Component {
  group = { name: '', description: '', image: '', leader: 0 };
